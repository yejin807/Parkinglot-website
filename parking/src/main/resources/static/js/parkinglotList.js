...
function getListItem(index, places) {
  ...
  itemStr +=
    '  <span class="tel">' + places.parkinglotTel + "</span>" + "</div>";
...
function displayPagination(keyword, respPagination) {
  ...
  for (i = respPagination.startPage; i <= respPagination.endPage; i++) {
    ...
    if (i === respPagination.currentPage) {
      el.className = "on mr-1 ml-1";
    } else {
      el.className = "mr-1 ml-1";
...
function displayInfowindow(marker, parkinglotInfo) {
  var content =
    ...
    parkinglotInfo.parkingName +
    "</div>" +
    '<div style="padding:5px;z-index:1;"> 기본 요금: ' +
    parkinglotInfo.basicFee +
    "</div>" +
    '<div style="padding:5px;z-index:1;"> 주차가능대수: ' +
    parkinglotInfo.currentCnt +
    "</div>" +
    '<div style="padding:5px;z-index:1;"> 종료 시간: ' +
    parkinglotInfo.closeTime +
    "</div>";
