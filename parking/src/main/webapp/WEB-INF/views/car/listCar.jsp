...
<div class="page">
	...
	onclick="location.href='/car/insert/${parkinglot.parkinglotId}'">입차등록</button>
			...
			<c:forEach items="${cars.content }" var="car" varStatus="st">
				<tr>
					...
					<c:choose>
						...
						<c:otherwise>
							<td><span id='spanID3${st.index}'></span> <script>minutes('${car.intime }' ,'spanID3${st.index}')</script>
								분</td>
							<td><span id='spanID4${st.index}'></span> <script>fee('${car.intime }' ,'spanID4${st.index}',${parkinglot.basicFee })</script>
								원</td>
							<td><button type="button" class="btn btn-danger"
